...
public class MyProgressBar extends View {
	...
	public MyProgressBar setDuration(int duration) {
		this.duration = duration;
		return this;
	}

	public MyProgressBar setBackDrawable(Drawable drawable) {
		setBackgroundDrawable(drawable);
		return this;
	}

	public MyProgressBar setForeDrawable(Drawable drawable) {
		fore = drawable;
		postInvalidate();
		return this;
	}

	public MyProgressBar setProgress(int progress) {
		if (progress < 0) {
			progress = 0;
		}
		if (progress > max) {
	...
	public MyProgressBar setMax(int max) {
		if (max < 0) {
			max = 1;
		}
		this.max = max;
	...
	public MyProgressBar show(int max) {
		setVisibility(View.VISIBLE);
		this.progress = 0;
		this.animProgress = 0;
		setMax(max);
