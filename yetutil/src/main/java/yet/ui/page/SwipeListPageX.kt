...
abstract class SwipeListPageX<T> : ListPage<T>() {
	...
	override fun unpackBindView(position: Int, itemView: View, item: T): View {
		if (itemView is XSwipeItemView) {
			onUpdateSwipeView(itemView, position, item)
			return super.unpackBindView(position, itemView.itemView, item)
		}
		return super.unpackBindView(position, itemView, item)
	}
