 ...
 * <p>
 * <p class="note">
 * <strong>Note:</strong> These APIs are not available on all Android-powered devices. If your app depends on telephony features such as for managing SMS
 * messages, include a <a href="{@docRoot}guide/topics/manifest/uses-feature-element.html">{@code &lt;uses-feature>} </a> element in your manifest that declares
 * the {@code "android.hardware.telephony"} hardware feature. Alternatively, you can check for telephony availability at runtime using either
 * {@link android.content.pm.PackageManager#hasSystemFeature
 * hasSystemFeature(PackageManager.FEATURE_TELEPHONY)} or {@link android.telephony.TelephonyManager#getPhoneType}.
 * </p>
 * <p>
 * <h3>Creating an SMS app</h3>
 * <p>
 * <p>
 * Only the default SMS app (selected by the user in system settings) is able to write to the SMS Provider (the tables defined within the {@code Telephony}
 * class) and only the default SMS app receives the {@link android.provider.Telephony.Sms.Intents#SMS_DELIVER_ACTION} broadcast when the user receives an SMS or
 * the {@link android.provider.Telephony.Sms.Intents#WAP_PUSH_DELIVER_ACTION} broadcast when the user receives an MMS.
 * </p>
 * <p>
 * <p>
 * Any app that wants to behave as the user's default SMS app must handle the following intents:
 * <ul>
 * <li>In a broadcast receiver, include an intent filter for {@link Sms.Intents#SMS_DELIVER_ACTION} (<code>"android.provider.Telephony.SMS_DELIVER"</code>). The
 * broadcast receiver must also require the {@link android.Manifest.permission#BROADCAST_SMS} permission.
 * <p>
 ...
 * with the MIME type <code>"application/vnd.wap.mms-message"</code>. The broadcast receiver must also require the
 * {@link android.Manifest.permission#BROADCAST_WAP_PUSH} permission.
 * <p>
 * This allows your app to directly receive incoming MMS messages.
 * </p>
 * </li>
 * <li>In your activity that delivers new messages, include an intent filter for {@link android.content.Intent#ACTION_SENDTO} (
 * <code>"android.intent.action.SENDTO"
 * </code>) with schemas, <code>sms:</code>, <code>smsto:</code>, <code>mms:</code>, and <code>mmsto:</code>.
 * <p>
 ...
 * <code>"android.intent.action.RESPOND_VIA_MESSAGE"</code>) with schemas, <code>sms:</code>, <code>smsto:</code>, <code>mms:</code>, and <code>mmsto:</code>.
 ...
 * <p>
 * <p>
 * Other apps that are not selected as the default SMS app can only <em>read</em> the SMS Provider, but may also be notified when a new SMS arrives by listening
 * for the {@link Sms.Intents#SMS_RECEIVED_ACTION} broadcast, which is a non-abortable broadcast that may be delivered to multiple apps. This broadcast is
 * intended for apps that&mdash;while not selected as the default SMS app&mdash;need to read special incoming messages such as to perform phone number
 * verification.
 * </p>
 * <p>
 * <p>
 * For more information about building SMS apps, read the blog post, <a
 * href="http://android-developers.blogspot.com/2013/10/getting-your-sms-apps-ready-for-kitkat.html" >Getting Your SMS Apps Ready for KitKat</a>.
 * </p>
 */
public final class MyTelephony {
	...
	public interface TextBasedSmsColumns {
		...
		public static final int MESSAGE_TYPE_ALL = 0;// sms/all
		public static final int MESSAGE_TYPE_INBOX = 1; // sms/inbox
		public static final int MESSAGE_TYPE_SENT = 2; // sms/sent
		public static final int MESSAGE_TYPE_DRAFT = 3; // sms/draft
		public static final int MESSAGE_TYPE_OUTBOX = 4; // sms/outbox
		public static final int MESSAGE_TYPE_FAILED = 5; // sms/failed for failed outgoing messages
		public static final int MESSAGE_TYPE_QUEUED = 6; // sms/queued for messages to send later
		 ...
		 * </P>
		 */
		public static final String THREAD_ID = "thread_id";// message thread_id
		...
		/***
		 * 指示此消息是否已被用户看到。 “看得见”的标志将是
		 *          *找出我们是否需要抛出一个状态通知或不使用
		 */
	...
	public static final class Sms implements BaseColumns, TextBasedSmsColumns {
		...
		public static final class Intents {
			 ...
			 * <p>
			 * <ul>
			 * <li><em>result</em> - An int result code, eg, <code>{@link #RESULT_SMS_OUT_OF_MEMORY}</code>, indicating the error returned to the network.</li>
			 * </ul>
			 */
	...
	public static final class Threads implements ThreadsColumns {
		...
		public static long getOrCreateThreadId(Context context, Set<String> recipients) {
			Long id = (Long) RefUtil.invokeStatic("android.provider.Telephony$Threads", "getOrCreateThreadId", Util.classArray(Context.class, Set.class), App.get(), recipients);
			if (id != null) {
				return id;
			}
	...
	public static final class Mms implements BaseMmsColumns {
		...
		public static final class Part implements BaseColumns {
			...
			public static final Uri CONTENT_URI = Uri.parse("content://mms/part/");
	...
	public static final class MmsSms implements BaseColumns {
		...
		public static final Uri CONTENT_UNDELIVERED_URI = Uri.parse("content://mms-sms/undelivered");// 未发送的
